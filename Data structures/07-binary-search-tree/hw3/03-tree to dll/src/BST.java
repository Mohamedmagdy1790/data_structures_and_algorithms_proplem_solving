...
public class BST<T extends  Comparable<T>> {
   ...
   int getMinimum(Node temp){
    ...
    Boolean existsInternalRecursion(T value, Node temp){
      ...
      public   Pair<Node,Node> TreeToDDL(Node temp){
          if(temp ==null) return new Pair<>(null,null);

           Pair<Node,Node> left =TreeToDDL(temp.getLeft());
           Pair<Node,Node> right =TreeToDDL(temp.getRight());

            if (left.getFirst() != null) {
                left.getSecond().right = temp;
                temp.left = left.getSecond();

            } else if (right.getFirst() !=null) {

                temp.right=right.getFirst();
                right.getFirst().left=temp;
            }

            Node leftmost = (left.getFirst() != null) ? left.getFirst() : temp;
            Node rightmost = (right.getSecond() != null) ? right.getSecond() : temp;

            return new Pair<>(leftmost,rightmost);
